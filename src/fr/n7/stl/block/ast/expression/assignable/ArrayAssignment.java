...
public class ArrayAssignment extends AbstractArray implements AssignableExpression {
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		int offset = ((VariableAssignment) this.array).declaration.getOffset();
		Register register = ((VariableAssignment) this.array).getDeclaration().getRegister();
		int size = this.getType().length();
		code.add(_factory.createLoad(register, offset, size));
		code.append(this.index.getCode(_factory));
		code.add(_factory.createLoadL(size));
		...
		code.add(_factory.createStoreI(size));
		return code;
	}
