...
public abstract class AbstractField implements Expression {
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		Boolean ok = this.record.fullResolve(_scope);
		...
		if(t instanceof NamedType ) {
			...
			if(((RecordType)t).contains(this.name)){
				this.field=((RecordType)t).get(this.name);
				return ok && true;
			}
			
		}
		Logger.error("Type Record error detected");
		return false;
	}
