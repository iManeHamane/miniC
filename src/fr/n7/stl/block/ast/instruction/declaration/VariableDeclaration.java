...
public class VariableDeclaration implements Declaration, Instruction {
	...
	public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
		...
		if(_scope.accepts(this)) {
			...
			ok = this.value.collectAndBackwardResolve(_scope);
			return ok;
		}
		else {
	...
	public boolean checkType() {
		Type t_dec = value.getType();
		if(!t_dec.compatibleWith(type)) {
			Logger.error("error check type declaration");
			return false;
		}
		return true;
	...
	public Fragment getCode(TAMFactory _factory) {
		...
		f.add(_factory.createPush(this.getType().length()));
		f.append(this.value.getCode(_factory));
		f.add(_factory.createStore(this.register, this.offset, this.getType().length()));
		return f;
